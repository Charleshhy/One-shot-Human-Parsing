...
class DeepLabv3_plus_v2(nn.Module):
    def __init__(self, nInputChannels=3, n_classes=21, os=16, pretrained=True, _print=True):
        ...
        self.midfea_adapt_conv = nn.Conv2d(2048, 256, 1, bias=False)
        self.midfea_adapt_bn = SynchronizedBatchNorm2d(256)

        self.midfea_conv = nn.Sequential(
            Decoder_module(304, 256),)

        self._dlab_init_weight()

        # self.semantic2 = nn.Conv2d(256, 2, kernel_size=1, stride=1)
    ...
    def forward(self, input):
        ...
        x = self.decoder3(self.decoder2(self.decoder1(x)))
        sem = self.semantic(x)
        sem = F.upsample(sem, size=input.size()[2:], mode='bilinear', align_corners=True)

        return sem, x

    def oneshot_flex_forward(self, input, feature_lvl='high'):
        mid_lvl_fea, low_level_features = self.xception_features(input)
        # print(x.size())
        x = mid_lvl_fea
        ...
        if feature_lvl == 'high':
            ...
            # print(x.size())
            x = torch.cat((x, low_level_features), dim=1)
            high_lvl_fea = self.decoder1(x)

            return high_lvl_fea

        elif feature_lvl == 'mid':
            mid_lvl_fea = self.midfea_adapt_conv(mid_lvl_fea)
            mid_lvl_fea = self.midfea_adapt_bn(mid_lvl_fea)
            mid_lvl_fea = self.relu(mid_lvl_fea)

            mid_lvl_fea = F.upsample(mid_lvl_fea, size=low_level_features.size()[2:], mode='bilinear',
                                     align_corners=True)

            low_level_features = self.feature_projection_conv1(low_level_features)
            low_level_features = self.feature_projection_bn1(low_level_features)
            low_level_features = self.relu(low_level_features)
            mid_lvl_fea = torch.cat((mid_lvl_fea, low_level_features), dim=1)
            mid_lvl_fea = self.midfea_conv(mid_lvl_fea)

            return mid_lvl_fea

        elif feature_lvl == 'mid_no_low':
            mid_lvl_fea = self.midfea_adapt_conv(mid_lvl_fea)
            mid_lvl_fea = self.midfea_adapt_bn(mid_lvl_fea)
            mid_lvl_fea = self.relu(mid_lvl_fea)
        ...
        elif feature_lvl == 'early_mid_no_low':
            x1 = self.aspp1(x)
            x2 = self.aspp2(x)
            x3 = self.aspp3(x)
            x4 = self.aspp4(x)
    ...
    def load_state_dict_from_parallel(self, state_dict):
        own_state = self.state_dict()
        #for name inshop_cos own_state:
        #    print name
        new_state_dict = OrderedDict()
    ...
    def load_state_dict_new_from_xception(self, state_dict):
        own_state = self.state_dict()
        #for name inshop_cos own_state:
        #    print name
        new_state_dict = OrderedDict()
        for name, param in state_dict.items():
            ...
            if 'xception_features' in name:
                name = name.replace('xception_features', 'backbone')
...
class AtrousPooling(nn.Module):
    """
    Reference:
        Zhao, Hengshuang, et al. *"Pyramid scene parsing network."*
    """

    def __init__(self, rates, in_channels=2048):
        super(AtrousPooling, self).__init__()

        rates = [1, 6, 12, 18]

        self.aspp1 = ASPP_module_rate0(in_channels, 256, rate=rates[0])
        self.aspp2 = ASPP_module(in_channels, 256, rate=rates[1])
        self.aspp3 = ASPP_module(in_channels, 256, rate=rates[2])
        self.aspp4 = ASPP_module(in_channels, 256, rate=rates[3])
